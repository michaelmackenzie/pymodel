...
def convert_combine_to_hfmodel(
    ...
    lines: List[str] = []
    lines.append("# Auto-generated hfmodel card converted from a Combine card")
...
def convert_hfmodel_to_combine(
    ...
    lines: List[str] = []
    lines.append("# Auto-generated Combine card converted from a hfmodel card")
    lines.append("imax *")
    lines.append("jmax *")
    lines.append("kmax *")
    lines.append("-")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Convert a Higgs Combine text card to a hfmodel card, or reverse-convert a hfmodel card "
    ...
    parser.add_argument(
        "--direction",
        choices=["auto", "combine-to-hfmodel", "hfmodel-to-combine"],
        default="auto",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Do not map Combine process labels using shapes templates when converting to hfmodel. "
...
def _default_root_file_from_hfmodel(parsed: ParsedCard) -> str:
    for shape in parsed.shapes:
        if shape.process.lower() == "data_obs":
            continue
...
def main() -> None:
    ...
    if direction == "auto":
        ...
        direction = "combine-to-hfmodel" if flavor == "combine" else "hfmodel-to-combine"

    parsed = parse_card(input_card)

    if direction == "combine-to-hfmodel":
        ...
        output_text = convert_combine_to_hfmodel(
            parsed,
        ...
        root_file = args.root_file or _default_root_file_from_hfmodel(parsed)
        output_text = convert_hfmodel_to_combine(
            parsed=parsed,
            root_file=root_file,
